...
<div className="p-3 sm:p-4 md:p-6 min-h-screen overflow-y-auto custom-scrollbar space-y-6 sm:space-y-8 bg-bg-body rounded-2xl">
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
    ...
    <div className="bg-bg-card p-4 sm:p-6 rounded-3xl border border-border flex items-center gap-3 sm:gap-4">
      <div className="p-3 sm:p-4 bg-bg-body text-text rounded-2xl flex-shrink-0"><FaListUl size={18} /></div>
      <div className="min-w-0 flex-1">
    ...
    <div className="overflow-x-auto custom-scrollbar">
      <table className="w-full text-left min-w-[600px]">
        ...
        <tbody className="divide-y divide-border">
          {loading ? (
            <tr>
              <td colSpan="5" className="px-6 py-8 text-center text-muted text-sm">Loading expenses...</td>
            </tr>
          ) : expenses.length > 0 ? (
            expenses.map((ex) => (
              <tr key={ex.id} className="hover:bg-bg-body transition-colors">
                <td className="px-6 py-4 text-sm font-bold text-text whitespace-nowrap">{new Date(ex.date).toLocaleDateString()}</td>
                ...
                <td className="px-6 py-4 text-right font-black text-text text-sm whitespace-nowrap">Rs {ex.amount.toLocaleString()}</td>
              </tr>
            ))
          ) : (
            <tr>
              <td colSpan="5" className="px-6 py-8 text-center text-muted text-sm">No expenses found for this period</td>
            </tr>
          )}
        </tbody>
      </table>
