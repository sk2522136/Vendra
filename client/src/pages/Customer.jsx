    ...
    <div className="flex-1 overflow-x-auto overflow-y-auto">
      <table className="w-full text-left min-w-full sm:min-w-[700px]">
        ...
        <tbody className="divide-y divide-border">
          {customers.map((c) => (
            <tr className="hover:bg-bg-body transition-colors group" key={c._id}>
              <td className="px-3 sm:px-6 py-3 sm:py-4">
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full text-text bg-bg-body border border-border flex items-center justify-center font-bold flex-shrink-0 text-xs sm:text-sm">
                    {c.name[0]}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-text text-xs sm:text-sm truncate">{c.name}</p>
                    <p className="text-[8px] sm:text-[10px] text-muted flex items-center gap-1 truncate"><FaPhoneAlt size={8}/> {c.phoneNumber}</p>
                  </div>
                </div>
              </td>
              ...
              <td className="px-3 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm text-text font-medium hidden lg:table-cell">
                {c.lastPaymentDate || 'No payments yet'}
              </td>
              <td className="px-3 sm:px-6 py-3 sm:py-4 text-right">
                <button 
                  onClick={() => handleViewBtn(c._id)}
                  className="p-1.5 sm:p-2 text-white bg-bg-primary hover:opacity-90 rounded-md sm:rounded-lg transition-all text-[8px] sm:text-[10px] font-bold uppercase inline-flex items-center gap-1 whitespace-nowrap"
                  title="View History"
                >
